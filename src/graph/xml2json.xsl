...
<xsl:text>{ 
    "data": [
                ...
                <xsl:for-each select="rra[pdp_per_row = $definition and not(preceding-sibling::rra[pdp_per_row = $definition])]/database/row">
                    <xsl:variable name="position" select="position()"/>
                    
                    <xsl:text>        {
</xsl:text>                    
                        <xsl:variable name="date" select="$lastupdate - count(following-sibling::row)*$step"/>
...
<xsl:text/>           "<xsl:value-of select="$cf"/>-<xsl:value-of select="normalize-space(/rrd/ds[count(preceding-sibling::ds) = ($num - 1)]/name)"/>": <xsl:value-of select="."/><xsl:if test="not($last1 and $last2)">,</xsl:if><xsl:text>
</xsl:text>                   
	                        </xsl:for-each>    
	                    </xsl:for-each>
